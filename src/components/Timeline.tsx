      ...
      {Object.entries(props.meetings).map(([key, value]) => 
        <li className="mb-10 ml-8 w-[300px]" key={key}>            
          ...
          <h3 className="flex items-center mb-1 text-lg font-semibold text-gray-900 dark:text-white">
            ...
            {
              value.datetime && <span 
                className={`${value.datetime > Date.now() ? 'bg-blue-100 text-blue-800 dark:bg-blue-200 dark:text-blue-800' : 'bg-red-100 text-red-800 dark:bg-red-200 dark:text-red-800'} text-sm font-medium mr-2 px-2.5 py-0.5 rounded ml-3`}>
                {value.datetime > Date.now() ? 'Soon' : 'Past'}
              </span>
            }
          </h3>
          {
            value.datetime && <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">
              {getLocaleDateStringFromLong(value.datetime)}
            </time>
          }
